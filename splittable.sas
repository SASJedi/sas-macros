...
%if %superq(table)= or %superq(column)= %then %do;
   %let MSGTYPE=ERROR;
   %put &MSGTYPE: &SYSMACRONAME Syntax Error ***********;
   %put &MSGTYPE- Table was "%superq(table)" / column was "%superq(column)";
...
%if %qupcase(%superq(keep)) ne DROP 
   ...
   %goto syntax;
%end;
%if not(%sysfunc(exist(%superq(table)))) %then %do;
   %let MSGTYPE=ERROR;
   %put &MSGTYPE: &SYSMACRONAME Syntax Error ***********;
...
%if %qscan(%superq(table),1)=%superq(table) %then %do;
   %let libname=WORK;
...
%end;
%if %superq(outlib)= %then %do;
   %let outlib=WORK;
%end;
%if %qupcase(%superq(keep)) = %then %do;
  %let keep=DROP;
%end;
...
%end;
%do i=1 %to &sqlobs;
   %if not(%sysfunc(nvalid(&prefix%superq(Table&i)))) %then %do;
      %let MSGTYPE=ERROR;
      %put &MSGTYPE: &SYSMACRONAME Syntax Error ***********;
...
%if %superq(type)=num %then %do i=1 %to &sqlobs;
      when (%superq(Table&i)) output %unquote(%superq(outlib).&prefix%superq(Table&i));
%end;
%else %do i=1 %to &sqlobs;
      when ("%superq(Table&i)") output %unquote(%superq(outlib).&prefix%superq(Table&i));
%end;
   end;
%if %qupcase(%superq(keep))=DROP %then %do;
   drop %superq(column);
%end;
run;
%mend SplitTable;
