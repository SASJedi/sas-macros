...
%if %SUPERQ(dsn)= %then %do;
   ...
   %goto Syntax;
%end;
%if %qupcase(%qsubstr(%superq(dsn),1,1))=!
   %then %do;
...
%syntax: %put;
   ...
   %PUT &MSGTYPE-     attribs=attributes to be modified (FIL);
   %PUT &MSGTYPE-         F=formats;
   %PUT &MSGTYPE-         I=informats;
   %PUT &MSGTYPE-         L=Labels;
   %PUT &MSGTYPE-         Default:FIL;
   %PUT ;
   %PUT &MSGTYPE-  Example: ;
   %PUT &MSGTYPE-  %NRSTR(%remove_metadata%(work.cars%));
   %PUT &MSGTYPE-  Removes all formats, informats and labels;
   %PUT ;
   %PUT &MSGTYPE-  %NRSTR(%remove_metadata%(work.cars,LI%));
   %PUT &MSGTYPE-  Removes all informats and labels;
   %PUT ;
   %PUT &MSGTYPE-  %NRSTR(%remove_metadata%(work.cars,L%));
   %PUT &MSGTYPE-  Removes all labels;
   %PUT ;
   %PUT ;
   %PUT &MSGTYPE-  *************************************************************;
   %RETURN;
